(ns cas-registry.main
  "Kotodama WASM entrypoint for the cas_registry clean-room actor (L4) — Clojure port.
  ...
  facts (`cas_registry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cas_registry")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cas_registry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cas_registry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cas_registry-compat/ready)
